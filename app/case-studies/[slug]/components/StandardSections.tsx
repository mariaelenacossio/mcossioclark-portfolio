  ...
  return (
    <>
      ...
      <section id="goal" className="scroll-mt-24">
        ...
        <div className="rounded-2xl border-l-[3px] border-coral bg-mist p-6">
          <p className="font-body text-caption uppercase tracking-widest text-coral mb-2">Hypothesis</p>
      ...
      <section id="research" className="scroll-mt-24">
        ...
        <ul role="list" className="flex flex-col gap-2 mb-8">
          {project.research.methods.map(m => (
            <li key={m} className="flex gap-3 font-body text-body text-ghost leading-[1.6]">
        ...
        <ol role="list" className="flex flex-col gap-3">
          {project.research.keyFindings.map((f, i) => (
            <li key={i} className="rounded-2xl border border-line bg-mist p-5">
              <div className="flex gap-4">
                <span className="font-display italic text-coral shrink-0">
                  0{i + 1}
                </span>
      ...
      <section id="insights" className="scroll-mt-24">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {project.insights.map((ins, i) => (
            <div key={i} className="rounded-2xl border border-line bg-mist p-6">
              <div className="flex items-center gap-2.5 mb-2.5">
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-coral font-body text-caption font-bold text-paper">
                  {i + 1}
                </span>
                <p className="font-display text-ink font-semibold">{ins.label}</p>
              </div>
              <p className="font-body text-body text-ghost leading-[1.6]">
                {ins.detail}
              </p>
            </div>
          ))}
      ...
      <section id="design" className="scroll-mt-24">
          ...
          {project.iterations.map((it, i) => (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="rounded-full bg-coral/10 px-2.5 py-1 font-body text-caption font-bold uppercase text-coral">
                  {it.version}
                </span>
                <p className="font-body font-semibold text-ink">{it.title}</p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <p className="eyebrow text-ghost mb-1.5">User insight</p>
                  <p className="font-body text-body text-ghost leading-[1.6] italic">
                    "{it.insight}"
                  </p>
                </div>
                <div>
                  <p className="eyebrow text-ghost mb-1.5">Design change</p>
                  <p className="font-body text-body text-ghost leading-[1.6]">
                    {it.change}
                  </p>
                </div>
        ...
        <div className="grid gap-5 md:grid-cols-3 mb-6">
          ...
          <div className="rounded-2xl border border-line bg-mist p-5">
            <p className="eyebrow text-ghost mb-3">Colour palette</p>
          ...
          {/* Typography */}
          <div className="rounded-2xl border border-line bg-mist p-5">
            <p className="eyebrow text-ghost mb-3">Typography</p>
            <p className="font-body text-body text-ghost leading-[1.6]">
          ...
          {/* Components */}
          <div className="rounded-2xl border border-line bg-mist p-5">
            <p className="eyebrow text-ghost mb-3">Key components</p>
            <ul role="list" className="flex flex-col gap-1.5">
              {project.designSystem.components.map(comp => (
                <li
                  key={comp}
                  className="flex gap-1.5 font-body text-caption text-ghost leading-snug"
                >
      ...
      <section id="learnings" className="scroll-mt-24">
        ...
        <ol role="list" className="flex flex-col gap-4">
          {project.learnings.map((l, i) => (
            <li key={i} className="flex gap-5 rounded-2xl border border-line bg-mist p-6">
              <span className="font-display text-[1.75rem] italic leading-none text-coral shrink-0">
                0{i + 1}
              </span>
